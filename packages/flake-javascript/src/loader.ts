#!/usr/bin/env node
import { OutputObject, Reasons, Properties } from "@flake-universal/core";
import { default as HTTPServer } from "./HttpServer.js";
import child_process from "node:child_process";

export default async function loader(): Promise<OutputObject> {
	return new Promise((resolve, reject) => {
		...
		if (options.server) {
			const serve = () => {
				...
				server.serve(filePath).then((data: OutputObject) => {
					resolve(data);
				})
			}
			...
			child_process.execFile("node", [filePath], (err: Error, stdout: string) => {
				if (err) {
					throw err;
				}

				try {
					let json: OutputObject = JSON.parse(stdout);

...
const hex = (color: string, bg : boolean = false): Function => {
	if (color.startsWith("#")) {
		color = hexToRGB(color);
	} else if (color.startsWith("\x1b")) {
		return (...text: any[]) => `${color}${text.join(" ")}\x1b[0m`;
	}
...
const bgHex = (color: string) => hex(color, true);

class ConsoleLogger {
	public indent: number;
	private _tab: string;
	private lastState : {tab: string, indent: number, color: string};
	private _color: string;
	constructor(indent: number = 0, tabChar: string = "    ", color: string = "\x1b[0m") {
		this._tab = tabChar;
		this.indent = indent;
		this._color = color;
		this.lastState = {
...
function displayTitle() {
	...
	log(
		`Tests: ${logTitle(
			`${stats.passed} passed`
		)}, ${stats.total} total, ${logTitle(ratio + "% Failed")}`
	);
}

function getSpinner(state : number) {
	let states = "⣾⣽⣻⢿⡿⣟⣯⣷"
...
function showTestsResults() {
	...
	if (arrExecuted) {
		// Always push results to a property on the window object to be accessed later on.
		if (window.hasOwnProperty("SnowblindTesting")) {
			window.SnowblindTesting(stats);
		}
		...
		for (const e of stats.describes) {
			...
			for (var i = 0; i < its.length; i++) {
				...
				for (var ii = 0; ii < _e.expects.length; ii++) {
					...
					log(
						`${status} ${expect.name} ${expect.async ? hex(legendItems["[async]"].color)("[async]") : ""}`
					);
				}
			}
		}

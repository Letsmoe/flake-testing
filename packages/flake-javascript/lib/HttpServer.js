...
export default class HTTPServer {
    constructor(config, port = 8097, root = process.cwd(), node_dir = path.join(process.cwd(), "/../node_modules/")) {
        this.config = config;
        this.port = port;
        this.root = root;
    ...
    send(data) {
        return new Promise((resolve, reject) => {
            ...
            data["identifier"] = id,
                Object.values(this.connections)[0].send(JSON.stringify(data));
        });
    }
    tryResolvePackage(url) {

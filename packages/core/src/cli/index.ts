...
(function () {
	...
	const TEMP_FOLDER = path.join(dir, FOLDER_NAME);
	 ...
	 */
	const watch = args.watch === false ? false : (this.config.watch || args.watch);
	...
	if (watch === true) {
		...
		ScanDirectory(this.dir, this.dir).forEach(async ([absFile, file]) => {
	...
	//if (config.localServer) {
		(async () => {
			const WebServerPort = await getPort();
					...
					if (task.loader) {
						...
						import(task.loader)
							.then(async (module) => {
								...
								const addFileExtension = (p: string, ext: string) => {
									if (ext) {
										return p + "." + ext
									}
									return p
								...
								utils.set = (option: Properties, value: any) => {

...
var parser = new colarg(process.argv.slice(2));
parser.addOption({
    name: "version",
    alias: "v",
    type: "any",
...
parser.addCommand({
    name: "snapshots",
    description: "Display a list of snapshots from the most recent test or a specific run.",
    args: [],
}, function () {
    process.exit();
});
parser.addCommand({
    name: "show",
    description: "Display a specific test run, picked by an index (use 'flake ls' for that) or the newest one.",
    args: [
        {
            name: "index",
            alias: "x",
...
parser.enableHelp();
export var args = parser.getArgs();
